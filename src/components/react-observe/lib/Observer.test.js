...
import Observer, { observe, context, subscribe } from '.'
...
describe('Observer', () => {
  ...
  it('should pass deps', () => {
    observer = new Observer(observe(2, 3, (a, b) => a + b))
    expect(observer.value).toBe(5)
  })

  it('should resolve deps', () => {
    const dep = observe(() => 3)
    observer = new Observer(observe(2, dep, (a, b) => a + b))
    expect(observer.value).toBe(5)
  })

  it('should recursively resolve deps', () => {
    const dep1 = observe(2, 1, (a, b) => a + b)
    const dep2 = observe(dep1, 1, (a, b) => a + b)
    const dep3 = observe(dep2, 1, (a, b) => a + b)
  ...
  it('should trigger onChange on first evaluation', () => {
    ...
    observer = new Observer(observe(() => 5), { onChange })
    expect(onChange).toHaveBeenCalledWith(5)
  })

  it('should trigger onChange when value changes during update', () => {
    const onChange = jest.fn()
    observer = new Observer(observe(() => 2), { onChange })
  ...
  it('should trigger onChange when deps change during update', () => {
    const onChange = jest.fn()
    observer = new Observer(observe(2, a => a), { onChange })
    observer.update(observe(5, a => a))
    expect(onChange).toHaveBeenLastCalledWith(5)
  })
  ...
  it("should not trigger onChange when deps don't change", () => {
    const onChange = jest.fn()
    observer = new Observer(observe(2, a => a), { onChange })
    observer.update(observe(2, a => a))
    expect(onChange).toHaveBeenCalledTimes(1)
  })

  it('should pass context to context variables', () => {
    const contextDep = context(c => c.value)
    observer = new Observer(contextDep, { context: { value: 5 } })
    expect(observer.value).toBe(5)
  })

  it('should subscribe to changes', () => {
    let subscription = mockSubscribe(Date.now())
    ...
    subscription = mockSubscribe(Date.now())
    onChange = jest.fn()
    observer = new Observer(subscription.dep, {
      onChange,
    })
